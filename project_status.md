...
**Currently working on:** Navigation now includes tracked-object targeting, current-room object listing, tutorial-objective resolution, transition metadata, connector interaction recovery, improved directional tracking with target-height pitch, proximity-based cadence, and proximity-based volume. The latest local passes hardened tutorial-objective resolution against live signpost text, widened current-room object discovery to cope with subdivided room-zone names, and normalized fine-grained runtime subzones such as `office6` onto the coarse navigation-graph zones such as `office` for pathfinding and same-room object tracking. The remaining work is concentrated on in-game validation plus broader gameplay/system coverage.
...
- Auto-walk guidance and directional object-tracker beeps for the tracked object or current navigation waypoint, with pitch tied to target height, cadence tied to proximity, and volume tied to proximity
...
- Verify in runtime that the updated object tracker gives usable left or right guidance, clear higher or lower pitch cues for target height, intuitive faster or slower cadence for proximity, and a helpful louder-as-you-approach volume curve without becoming confusing during transition interactions.
