...
- Everything in the crawlspace can now be reached with auto-walk. The Key,
  ...
  down the ladder puts you inside. Objective guidance that points you at the
  Key or the rat trap when you are in the crawlspace now works as intended.
- Fixed the smoke alarms (Arma). The two in the kitchen share the same internal
  name, and the mod was treating them as one object located in mid-air between
  them, which meant no route to either and nothing to see when the camera was
  on them. They are now nine separate alarms in their real positions, and all
  nine can be routed to. Meeting any one of them is enough, since they are all
  the same character.

WHAT'S NEW IN 1.1.3
===================
- Fixed three datables that could be selected but reported "no path": the
  coffee machine (Kopi), the toaster (Miranda) and the bobby pin (Bobby). Small
  objects resting on a counter or desk were being aimed at from underneath, so
  the counter itself blocked the line of sight from every standing spot. They
  now route normally.
- The coffee machine needed two further fixes: it reports its position at the
  wrong place in the house, and it is made of nine separate pieces that all
  count as the same character. Auto-walk now locates it correctly and, when one
  piece cannot be reached, routes to another piece of the same object.
- Objects that move as the story progresses (the bobby pin, the time capsule)
  no longer appear several times in the known objects list. Only the location
  the object is actually in is listed. Objects that genuinely exist in
  multiples, such as air vents and boxes, are unaffected.
- Pose card descriptions are no longer lost if you switch away from the game
  while a card is on screen. The description is now spoken when you return,
  instead of being dropped once the game had been in the background too long.
